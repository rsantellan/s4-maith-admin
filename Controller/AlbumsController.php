...
use  Maith\Common\AdminBundle\Services\AlbumCacheService;
...
class AlbumsController extends AbstractController
{
     ...
     * @param mAlbumRepository $mAlbumRepository
     * @return JsonResponse
     */
    public function refreshAlbumAction(Request $request, mAlbumRepository $mAlbumRepository)
    {
        $listmode = $request->request->get("list", false);
        $albumId = $request->request->get("id");
        $album = $mAlbumRepository->find($albumId);
     ...
     * @return Response
     */
    public function sortAlbumAction(mAlbumRepository $mAlbumRepository, $id)
    {
        $album = $mAlbumRepository->find($id);
        return $this->render('MaithCommonAdminBundle:Albums:fileSortable.html.twig', array('album' => $album));
    }

    /**
     * @param Request $request
     * @param AlbumCacheService $cacheService
     * @param mAlbumRepository $mAlbumRepository
     * @return JsonResponse
     * @throws \Psr\Cache\InvalidArgumentException
     */
    public function doSortAlbumAction(Request $request, AlbumCacheService $cacheService, mAlbumRepository $mAlbumRepository)
    {
        ...
        $cacheService->deleteAlbumAvatar($album);
        $response = new JsonResponse();
        $response->setData(array('output' => true));
        return $response;
    }

    public function removeFileAction(AlbumCacheService $cacheService, $id)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        if (!$file) {
            ...
            $cacheService->deleteAlbumAvatar($file->getAlbum());
     ...
     * @throws \Psr\Cache\InvalidArgumentException
     */
    public function onlineVideoFormAction(Request $request, AlbumCacheService $cacheService, $id)
    {
        $myFile = new mFile();
        $form = $this->createForm(mFileVideoType::class, $myFile, array(
    ...
    public function doFormUploadAction(Request $request, AlbumCacheService $cacheService, mAlbumRepository $mAlbumRepository)
    {
        ...
        $targetDir = $this->get('kernel')->getProjectDir() .  DIRECTORY_SEPARATOR . 'web' . DIRECTORY_SEPARATOR . $sf_targetDir;
        ...
        if ($movedFile) {
            $em = $this->getDoctrine()->getManager();
            $myFile = new mFile();
            $myFile->setAlbum($mAlbumRepository->find($albumId));
            $myFile->setName($name);
            ...
            $em->persist($myFile);
            $em->flush();
            $cacheService->deleteAlbumAvatar($myFile->getAlbum());
    ...
    public function showFileEditionAction($id)
    {
        ...
        $form = $this->createForm(mFileType::class, $file);
